...
let my_fav_num = 7; //Assign the value 7 to the string variablr my_fav_num...
...
console.log("good news friends I just found a new bigger table so can enjoy with three more friends!!!");
...
let _language_ = "English,Urdu,French,Italian,Japanese";
let listed_elements = _language_.split(",");
...
let cars = '{"company":"Toyota","model":"2022","name":"Corolla"}';
...
let new_users = ["Hamza", "Rafay", "ahad", "umer", "maaz"];
for (let _element of new_users) {
    if (current_users.includes(_element)) {
        console.log(`The user will need to enter new username as ${_element} is also present in the current_users`);
    }
    else
...
for (let b = 0; b <= pizza.length - 1; b++) {
    console.log(pizza[b]);
}
for (let a = 0; a <= 2; a++) {
...
function make_Shirt(size = "large", text = "Eat,Sleep,Code,trade and repeat") {
